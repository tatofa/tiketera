...
function fallbackEventCode(event: Event) { return String(event.eventCode || event.slug || event.id.slice(0, 8)).trim().toUpperCase(); }
function fallbackEventKey(event: Event) { return String(event.eventKey || event.id.slice(0, 6)).trim().toUpperCase(); }

function toDbEvent(event: Event, userId: string, producerId: string) {
  return { id: event.id, producer_id: producerId, name: event.name, slug: event.slug, description: event.description, image_url: event.imageUrl, status: event.status, capacity: event.capacity, created_by: userId, event_code: fallbackEventCode(event), event_key: fallbackEventKey(event), event_type: event.eventType ?? null, category: event.category ?? null, organizer_name: event.organizerName ?? null, artist_name: event.artistName ?? null, summary: event.summary ?? null, purchase_message: event.purchaseMessage ?? null, age_restriction: event.ageRestriction ?? null, province: event.province ?? null, locality: event.locality ?? null, address: event.address ?? null, access_policy: event.accessPolicy ?? null, terms_and_conditions: event.termsAndConditions ?? null };
}
...
export async function getSupabaseSession() { const supabase = createBrowserSupabaseClient(); if (!supabase) return { supabase: null, userId: null, token: null }; const { data } = await supabase.auth.getSession(); return { supabase, userId: data.session?.user?.id ?? null, token: data.session?.access_token ?? null }; }
