...
type CheckoutItemInput = { ticketTypeId: string; quantity: number };
...
function bundleSize(ticketType: any) {
  const n = Math.round(Number(ticketType?.promo_bundle_size ?? 1));
  const safe = Math.min(9, Math.max(1, Number.isFinite(n) ? n : 1));
  return ticketType?.is_promo ? safe : 1;
}
function normalizeItems(body: any): CheckoutItemInput[] {
  const rawItems: any[] = Array.isArray(body?.items) ? body.items : [];
  const normalized: CheckoutItemInput[] = rawItems.map((item: any) => ({ ticketTypeId: String(item?.ticketTypeId ?? '').trim(), quantity: Math.max(1, Number(item?.quantity ?? 1)) })).filter((item: CheckoutItemInput) => item.ticketTypeId.length > 0 && item.quantity > 0);
...
async function resolveBuyerUserId(input: { request: Request; supabaseUrl: string; anonKey?: string; adminClient: any; buyerEmail: string }) {
  ...
  if (token && input.anonKey) {
    const userClient = createClient(input.supabaseUrl, input.anonKey, { global: { headers: { Authorization: `Bearer ${token}` } }, auth: { autoRefreshToken: false, persistSession: false } });
    const { data } = await userClient.auth.getUser();
    if (data.user?.id) return data.user.id;
  }
  if (input.buyerEmail) {
    const { data: profile } = await input.adminClient.from('profiles').select('id,email').ilike('email', input.buyerEmail).maybeSingle();
...
export async function POST(request: Request) {
  ...
  if (!eventId || !eventDateId || !itemsInput.length || !buyerName || !buyerEmail) return NextResponse.json({ error: 'Faltan datos para crear la orden.' }, { status: 400 });
  ...
  const { data: eventDate, error: dateError } = await supabase.from('event_dates').select('id,event_id,status,event:events(id,name,producer_id,status)').eq('id', eventDateId).eq('event_id', eventId).single();
  ...
  const { data: ticketTypes, error: ticketError } = await supabase.from('ticket_types').select('id,event_id,sector_id,name,price,currency,max_per_order,capacity,is_promo,promo_bundle_size,status').eq('event_id', eventId).in('id', ticketIds);
  ...
  for (const item of items) {
    const qrQty = item.quantity * bundleSize(item.ticketType);
    ...
    const capacity = item.ticketType.capacity == null ? null : Number(item.ticketType.capacity);
    if (capacity !== null && Number.isFinite(capacity)) {
      const { count, error: countError } = await supabase.from('tickets').select('id', { count: 'exact', head: true }).eq('event_id', eventId).eq('ticket_type_id', item.ticketType.id).not('status', 'in', '(cancelled,refunded)');
      if (countError) return NextResponse.json({ error: countError.message }, { status: 400 });
      const sold = count ?? 0;
      if (sold + qrQty > capacity) return NextResponse.json({ error: `La entrada ${item.ticketType.name} está agotada o no tiene suficiente disponibilidad.` }, { status: 409 });
    }
  }
  let promoterLinkId: string | null = null;
  if (rrppCode) {
    const { data: link } = await supabase.from('promoter_links').select('id,event_id,active').eq('code', rrppCode).eq('event_id', eventId).eq('active', true).maybeSingle();
    promoterLinkId = link?.id ?? null;
  }
  const { data: feeRules } = await supabase.from('service_fee_rules').select('percentage,fixed_amount,min_fee,max_fee,currency,created_at').eq('active', true).eq('channel', channel).order('created_at', { ascending: false });
  ...
  const purchasedUnits = items.reduce((sum: number, item) => sum + item.quantity, 0);
  const currency = items[0]?.ticketType?.currency ?? 'ARS';
  const baseOrder = { user_id: buyerUserId, producer_id: event.producer_id, buyer_name: buyerName, buyer_email: buyerEmail, status: 'paid', subtotal_amount: subtotal, service_fee_amount: serviceFee, discount_amount: 0, total_amount: total, currency, channel };
  ...
  if (orderInsert.error && promoterLinkId && orderInsert.error.message.toLowerCase().includes('promoter')) orderInsert = await supabase.from('orders').insert(baseOrder as any).select('id,user_id').single();
  ...
  const itemRows = items.map((item) => ({ order_id: order.id, event_id: eventId, event_date_id: eventDateId, ticket_type_id: item.ticketType.id, quantity: item.quantity, unit_price: Number(item.ticketType.price ?? 0), service_fee_unit: purchasedUnits > 0 ? Math.round(serviceFee / purchasedUnits) : 0 }));
  const { error: itemError } = await supabase.from('order_items').insert(itemRows);
  if (itemError) return NextResponse.json({ error: itemError.message }, { status: 400 });
  const ticketRows = items.flatMap((item) => Array.from({ length: item.quantity * bundleSize(item.ticketType) }).map(() => ({ order_id: order.id, event_id: eventId, event_date_id: eventDateId, ticket_type_id: item.ticketType.id, sector_id: item.ticketType.sector_id, status: 'valid', holder_name: buyerName, holder_email: buyerEmail })));
