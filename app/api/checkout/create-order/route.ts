...
export async function POST(request: Request) {
  ...
  const { data: ticketTypes, error: ticketError } = await supabase.from('ticket_types').select('id,event_id,sector_id,name,price,currency,max_per_order,capacity,status').eq('event_id', eventId).in('id', ticketIds);
  ...
  for (const item of items) {
    ...
    if (capacity !== null && Number.isFinite(capacity)) {
      ...
      const sold = count ?? 0;
      if (sold + item.quantity > capacity) return NextResponse.json({ error: `La entrada ${item.ticketType.name} está agotada o no tiene suficiente disponibilidad.` }, { status: 409 });
    }
  }
  ...
  const itemRows = items.map((item) => ({ order_id: order.id, event_id: eventId, event_date_id: eventDateId, ticket_type_id: item.ticketType.id, quantity: item.quantity, unit_price: Number(item.ticketType.price ?? 0), service_fee_unit: totalQty > 0 ? Math.round(serviceFee / totalQty) : 0 }));
  const { error: itemError } = await supabase.from('order_items').insert(itemRows);
  if (itemError) return NextResponse.json({ error: itemError.message }, { status: 400 });

  const ticketRows = items.flatMap((item) => Array.from({ length: item.quantity }).map(() => ({ order_id: order.id, event_id: eventId, event_date_id: eventDateId, ticket_type_id: item.ticketType.id, sector_id: item.ticketType.sector_id, status: 'valid', holder_name: buyerName, holder_email: buyerEmail })));
