...
type TicketTypeRow = { id: string; event_id: string; name: string; price: number; status: string; max_per_order: number; capacity?: number | null };
type TicketRow = { id: string; event_id: string; ticket_type_id: string; status: string };
...
type ReportState = { loading: boolean; error: string; events: EventRow[]; ticketTypes: TicketTypeRow[]; tickets: TicketRow[]; sales: SalesRow[] };
...
function ReportesContent(){
 const [state,setState]=useState<ReportState>({loading:true,error:'',events:[],ticketTypes:[],tickets:[],sales:[]});
 const [selected,setSelected]=useState('');

 useEffect(()=>{
  async function load(){
   const supabase=createBrowserSupabaseClient();
   if(!supabase){setState({loading:false,error:'Falta configurar Supabase.',events:[],ticketTypes:[],tickets:[],sales:[]}); return;}
   const {data:sessionData}=await supabase.auth.getSession();
   if(!sessionData.session){setState({loading:false,error:'Sesión vencida. Volvé a ingresar.',events:[],ticketTypes:[],tickets:[],sales:[]}); return;}

   const [eventsRes,ticketsRes,issuedTicketsRes,salesRes]=await Promise.all([
    supabase.from('events').select('id,name,status,capacity,created_at').order('created_at',{ascending:false}),
    supabase.from('ticket_types').select('id,event_id,name,price,status,max_per_order,capacity').order('sale_start',{ascending:false}),
    supabase.from('tickets').select('id,event_id,ticket_type_id,status'),
   ...
   if(eventsRes.error){setState({loading:false,error:eventsRes.error.message,events:[],ticketTypes:[],tickets:[],sales:[]}); return;}
   const events=(eventsRes.data ?? []) as EventRow[];
   const ticketTypes=(ticketsRes.data ?? []) as TicketTypeRow[];
   const issuedTickets=(issuedTicketsRes.data ?? []) as TicketRow[];
   const sales=(salesRes.data ?? []) as SalesRow[];
   const error=ticketsRes.error?.message || issuedTicketsRes.error?.message || salesRes.error?.message || '';
   setState({loading:false,error,events,ticketTypes,tickets:issuedTickets,sales});
 ...
 const eventTickets=useMemo(()=>state.tickets.filter((row: TicketRow)=>row.event_id===selected && !['cancelled','refunded'].includes(row.status)),[state.tickets,selected]);
 ...
 const eventTotals=useMemo(()=>{
  ...
  const tickets=eventTickets.length;
  const gross=Array.from(uniqueOrders.values()).reduce((sum:number,row:SalesRow)=>sum+asNumber(row.subtotal_amount),0);
 ...
 },[eventRows,eventTickets]);

 const byTicketType=useMemo(()=>{
  return eventTicketTypes.map((ticket: TicketTypeRow)=>{
   const sold=eventTickets.filter((issued: TicketRow)=>issued.ticket_type_id===ticket.id).length;
   const gross=sold*asNumber(ticket.price);
   const capacity=ticket.capacity == null ? null : asNumber(ticket.capacity);
   const available=capacity == null ? null : Math.max(capacity-sold,0);
   const status=capacity != null && sold>=capacity ? 'sold_out' : ticket.status;
   return {id:ticket.id,name:ticket.name,price:asNumber(ticket.price),sold,gross,status,capacity,available};
  }).sort((a,b)=>b.sold-a.sold || b.gross-a.gross);
 },[eventTicketTypes,eventTickets]);
