...
import { Gift, Mail, Plus, Save, Ticket, Trash2, Wand2 } from 'lucide-react';
...
function TicketsContent() {
  ...
  useEffect(() => {
    ...
    const firstEvent = loadedEvents.find((event) => event.id === eventFromUrl)?.id ?? loadedEvents[0]?.id ?? '';
    const firstTicket = normalized.find((ticket) => ticket.eventId === firstEvent)?.id ?? normalized[0]?.id ?? '';
  ...
  function removeTicket(id: string) {
    if (eventTickets.length <= 1) return;
    const nextSelected = eventTickets.find((ticket) => ticket.id !== id)?.id ?? '';
    setTickets((current) => current.filter((ticket) => ticket.id !== id));
    setSelectedId(nextSelected);
    setCourtesy((current) => ({ ...current, ticketId: nextSelected }));
  }
  ...
  function sendCourtesy() {
    ...
    if (!courtesy.email || !ticket) {
      toast.error('Completá email y seleccioná el tipo de ticket.');
      return;
    }
    toast.success(`Cortesía de ${ticket.name} preparada para ${courtesy.email}. Cuando conectemos email transaccional se envía automáticamente.`);
