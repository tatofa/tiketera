...
function VentasContent(){
 ...
 useEffect(()=>{async function load(){const [salesResult,eventsResult]=await Promise.all([loadSalesReport(),loadEventsFromSupabase()]); setRows(salesResult.rows as SalesRow[]); setEvents(eventsResult.events.map(e=>({id:e.id,name:e.name}))); setError(salesResult.ok&&eventsResult.ok?'':salesResult.error||eventsResult.error||'No se pudieron cargar ventas.'); setLoading(false);} load();},[]);
 const filtered=rows.filter(row=>(!eventFilter||row.event_id===eventFilter)&&(!channelFilter||row.channel===channelFilter));
