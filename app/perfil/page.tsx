...
import { BadgeCheck, BarChart3, Link2, Ticket } from 'lucide-react';
...
function PerfilContent(){
 ...
 const roleSet=useMemo(()=>new Set(profile.roles.split(' · ').filter(Boolean)),[profile.roles]);
 const canOperate=roleSet.has('super_admin')||roleSet.has('admin')||roleSet.has('producer');
 if(loading) return <section className="container-page py-10"><div className="card p-6">Cargando perfil...</div></section>;
 return <section className="container-page py-10"><p className="font-semibold text-red-300">Mi cuenta</p><h1 className="text-4xl font-black text-white">Mi perfil</h1><p className="mt-2 max-w-2xl text-white/65">Desde acá podés revisar tu usuario. Los roles los asigna un productor o administrador.</p><div className="mt-8 grid gap-6 lg:grid-cols-[1fr_.9fr]"><div className="card p-6"><h2 className="text-xl font-black text-white">Datos del usuario</h2><div className="mt-5 grid gap-4"><div><label className="label">Nombre</label><input className="input mt-1 opacity-70" value={profile.fullName} readOnly/></div><div><label className="label">Email</label><input className="input mt-1 opacity-70" value={profile.email} readOnly/></div><div><label className="label">Telefono</label><input className="input mt-1 opacity-70" value={profile.phone||'Sin cargar'} readOnly/></div><div><label className="label">Roles activos</label><div className="mt-1 rounded-xl border border-white/15 bg-white/10 p-3 text-sm font-bold text-white">{profile.roles}</div></div></div></div><div className="space-y-6"><div className="card p-6"><h2 className="text-xl font-black text-white">Seguridad</h2><p className="mt-2 text-sm text-white/60">El cambio de clave se va a conectar como accion segura de Supabase en el siguiente paso.</p><a className="btn-secondary mt-5 w-full" href="/auth/login">Reingresar</a></div>{canOperate&&<div className="card p-6"><h2 className="text-xl font-black text-white">Menú productor / operador</h2><p className="mt-2 text-sm text-white/60">Accesos rápidos para operar eventos reales.</p><div className="mt-5 grid gap-2"><Link className="btn-secondary justify-start" href="/admin/tickets"><Ticket size={18} className="mr-2"/>Tickets y e-tickets</Link><Link className="btn-secondary justify-start" href="/admin/rrpp"><Link2 size={18} className="mr-2"/>RRPP</Link><Link className="btn-secondary justify-start" href="/admin/acreditacion"><BadgeCheck size={18} className="mr-2"/>Acreditación</Link><Link className="btn-secondary justify-start" href="/admin/reportes"><BarChart3 size={18} className="mr-2"/>Reportes</Link></div></div>}</div></div></section>;
}
